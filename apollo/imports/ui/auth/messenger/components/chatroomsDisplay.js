...
class ChatroomDisplay extends Component {
    ...
    render() {
        return (
            <Fragment>    
                {this.state.name ? <div onClick={this.roomSelected} className="chatRoomBloc"><img className="imgDisplayChat" src={this.state.coverUrl}/><h2>{this.state.name}</h2> {this.state.count ? <h4>{this.state.count} new</h4> :(null)} {this.state.status ? <img className="onlineStatusC" src="https://res.cloudinary.com/dkt7hv91e/image/upload/v1554764836/1244954.png"/> : <img className="onlineStatusC" src="https://res.cloudinary.com/dkt7hv91e/image/upload/v1554764836/red-dot-clip-art_18453.png"/>}</div> :(null)}
            </Fragment>
        )
    }
}
